...
import com.ymsli.bank.inhouse.exceptions.BankException;
import com.ymsli.bank.inhouse.exceptions.InsufficientBalanceException;
...
public class TransactionServiceImpl implements TransactionService {
    ...
    public void withdraw(String accountNumber, Double amount, Long clerkId) {
        ...
        if(amount== null || amount<=0) {
        	throw new BankException("Invalid withdrawl");
        }
	...
	@Override
	public void approveTransaction(Long transactionId, Long managerId, boolean approved) {
		 Transaction t = transactionRepository.findById(transactionId)
	                .orElseThrow(() -> new ResourceNotFoundException("Transaction not found"));

	        if (t.getStatus() != TransactionStatus.PENDING_APPROVAL)
	            throw new InvalidTransactionStateException("No transactions are pending");
	        ...
	        if (t.getType() == TransactionType.WITHDRAWAL) {
	            Account a = accountRepository.findByAccountNumber(t.getAccountNumber())
	        ...
	        if (t.getType() == TransactionType.TRANSFER) {
	            Account f = accountRepository.findByAccountNumber(t.getFromAccountNumber()).get();
	            Account to = accountRepository.findByAccountNumber(t.getToAccountNumber()).get();
